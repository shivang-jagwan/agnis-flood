...
export default function ControlledMetricsBar() {
  ...
  const waterLevel = floodState?.max_flood_level ?? 2.6
  const floodAreaPct = latestObservation?.flood_area_percent ?? (floodState?.total_flooded_cells ? Math.round((floodState.total_flooded_cells / 256) * 100) : (isRunning ? 42 : 0))
  const popRisk = floodState?.projected_lives_at_risk ?? (isRunning ? 8420 : 0)
  const livesSaved = floodState?.lives_saved ?? (isRunning ? 5740 : 0)

  return (
    <div className="bg-[#0f172a]/95 border border-emerald-500/40 rounded-2xl p-4 shadow-2xl backdrop-blur-md text-white font-sans select-none grid grid-cols-5 divide-x divide-slate-800 text-center">
      <div className="px-3">
      ...
      <div className="px-3">
        <div className="text-sm md:text-base font-mono font-black text-emerald-400 leading-tight">
          AEGIS ACTIVE
        </div>
        <div className="text-[11px] font-sans font-bold text-slate-400 uppercase tracking-wider mt-1">
          STATUS
