...
export default function SimulationMetricsBar() {
  ...
  const waterLevel = floodState?.max_flood_level ?? (sc ? sc.river_baseline : 2.4)
  ...
  return (
    <div className="bg-[#0f172a]/95 border border-slate-800/80 rounded-2xl p-4 shadow-2xl backdrop-blur-md text-white font-sans select-none grid grid-cols-4 divide-x divide-slate-800 text-center">
      <div className="px-3">
        ...
        <div className="text-xs font-sans font-bold text-slate-400 uppercase tracking-wider mt-1">
          WATER LEVEL
        </div>
      </div>
      ...
      <div className="px-3">
        <div className={`text-2xl md:text-3xl font-mono font-black ${isRunning ? 'text-red-400' : 'text-emerald-400'}`}>
          {isRunning ? 'CRITICAL' : 'NORMAL'}
        </div>
        <div className="text-xs font-sans font-bold text-slate-400 uppercase tracking-wider mt-1">
          FLOOD STATUS
        </div>
